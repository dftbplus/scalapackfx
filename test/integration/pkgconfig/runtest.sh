#!/bin/bash
#
# Tests whether the installed ScalapackFx library can be used with pkg-config based builds.
#
# Arguments:
#
...
#     the installed ScalapackFx tree.
#
...
pkg-config --exists scalapackfx || { echo "No PKG-CONFIG found for ScalapackFx" >&2;  exit 1; }

cflags=$(pkg-config --cflags scalapackfx)
libs=$(pkg-config --libs scalapackfx)
